...
def ensure_api_key():
    """
    Accepts either provider. Personas and simulations need a key; upload,
    column mapping and clustering all work without one, so a missing key is
    a warning rather than a hard stop.
    """
    existing = {}
    ...
    anth = existing.get("ANTHROPIC_API_KEY", "")
    orkey = existing.get("OPENROUTER_API_KEY", "")

    if orkey:
        say(f"OpenRouter key found ({orkey[:12]}…).")
        return
    if anth:
        say(f"Anthropic key found ({anth[:12]}…).")
        return

    head("API key needed")
    print("""
  Personas and simulations call a language model, so a key is required for
  ...
  Either provider works:
    ...
    2  OpenRouter  https://openrouter.ai/keys                    (sk-or-...)

  Paste a key below, or press Enter to skip for now.
""", flush=True)
    try:
    ...
    if not entered:
        say("Skipping. Add a key to .env later and restart.")
        if not os.path.isfile(ENVF):
            with open(ENVF, "w", encoding="utf-8") as f:
                f.write("ANTHROPIC_API_KEY=\n")
        return

    # the prefix tells us which provider it belongs to
    if entered.startswith("sk-or"):
        var, extra = "OPENROUTER_API_KEY", "LAUNCHGUARD_MODEL=anthropic/claude-sonnet-4.5\n"
        say("Detected an OpenRouter key.")
    else:
        var, extra = "ANTHROPIC_API_KEY", ""
        say("Detected an Anthropic key.")
