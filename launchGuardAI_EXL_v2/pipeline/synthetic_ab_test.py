...
# --- sample size policy ---------------------------------------------------
# The user gives a TOTAL sample size; it is divided across personas IN
# PROPORTION to each persona's share of the customer base. A 100-customer base
# split 60 / 25 / 15, sampled at 50, allocates 30 / 13 / 7.
#
# 120 is comfortably past the point where extra samples stop changing the
# answer, because within-segment variance is low by construction.
DEFAULT_SAMPLE_SIZE = 120

# No persona may be sampled below this, whatever its share of the base. A
# segment holding 2% of customers would otherwise round to zero rows and
...
def allocate_sample(sample_size: int, persona_sizes: Dict[str, int],
                    available: Dict[str, int]) -> Dict[str, int]:
    """
    Divide a total sample across personas in PROPORTION to each persona's
    share of the customer base.

    Uses the largest-remainder method so the allocated parts sum exactly to
    the requested total rather than drifting on rounding. Worked example:
    a 100-customer base split 60 / 25 / 15, sampled at 50, allocates 30 / 13 / 7.

    Every persona is guaranteed at least MIN_PER_PERSONA rows, because a
    segment sampled zero times would silently vanish from the breakdown.

    NOTE: proportional sampling means the raw sample already mirrors the
    population, so aggregation must NOT then re-weight by segment size on top
    of it - see aggregate() for the inverse-probability correction that keeps
    both sampling modes correct.
    """
    total_customers = sum(persona_sizes.values()) or 1
    names = list(persona_sizes.keys())

    sample_size = max(len(names) * MIN_PER_PERSONA, int(sample_size))
    sample_size = min(sample_size, sum(available.values()))

    exact = {n: sample_size * persona_sizes[n] / total_customers for n in names}
    alloc = {n: int(exact[n]) for n in names}

    # hand out the leftover seats to the largest fractional remainders,
    # breaking ties toward the larger segment
    remaining = sample_size - sum(alloc.values())
    order = sorted(names, key=lambda n: (-(exact[n] - int(exact[n])),
                                         -persona_sizes[n]))
    for n in order[:max(0, remaining)]:
        alloc[n] += 1

    # floors and ceilings, applied after allocation
    for n in names:
        alloc[n] = max(MIN_PER_PERSONA, alloc[n])
        alloc[n] = min(alloc[n], available[n])

    return alloc


def split_sample(sample_size: int, n_personas: int, available: int) -> tuple:
    """Approximate figures for the pre-run estimate, which does not yet know
    the per-persona sizes. Actual allocation happens in allocate_sample()."""
    n_personas = max(1, n_personas)
    sample_size = max(n_personas * MIN_PER_PERSONA, int(sample_size))
    sample_size = min(sample_size, available)
    return max(MIN_PER_PERSONA, sample_size // n_personas), sample_size
...
def estimate_run(n_personas: int, sample_size: int, available: int,
    ...
    return {
        ...
        # this is an approximation: the real allocation is proportional and
        # is computed once the per-persona sizes are known
...
def sample_users(df: pd.DataFrame, allocation: Dict[str, int],
                 seed: int = 42) -> pd.DataFrame:
    """Draw the allocated number of rows from each persona."""
    if "persona_name" not in df.columns:
        raise ValueError("users file has no persona_name column")

    parts = []
    for name, grp in df.groupby("persona_name"):
        n = min(len(grp), allocation.get(name, 0))
        if n > 0:
            parts.append(grp.sample(n=n, random_state=seed))
    if not parts:
        raise ValueError("sample allocation produced no rows")
...
def evaluate_user(sample: dict, variant_a: str, variant_b: str,
    ...
    for attempt in range(3):
        ...
        try:
            text = llm.call(
                system, build_user_prompt(meta, variant_a, variant_b),
                model=model, max_tokens=2000, step="abtest",
...
def _insight(s: dict, personas: List[dict], noise: Optional[float]) -> str:
    """
    One plain sentence interpreting the headline numbers.

    Deterministic on purpose - rules over the computed figures, not another
    model call. It is instant, free, and says the same thing every time the
    numbers are the same.
    """
    rpu, cvr, aov = (s["rpu_relative_lift_pct"], s["cvr_relative_lift_pct"],
                     s["aov_relative_lift_pct"])

    if noise is not None and abs(rpu) <= abs(noise):
        return (f"This {rpu:+.1f}% movement sits at or below the measured "
                f"{abs(noise):.1f}% noise floor, so it is not distinguishable "
                f"from no effect at all.")

    if abs(rpu) < 1.0:
        return "Revenue per user barely moves. This change is close to neutral across the base."

    direction = "lifts" if rpu > 0 else "reduces"
    parts = [f"Revenue per user {direction} by {abs(rpu):.1f}%"]

    # which lever is actually doing the work
    if abs(aov) > abs(cvr) * 1.6:
        parts.append("driven mainly by basket size rather than conversion - the "
                     "same shoppers are spending more, rather than more shoppers buying")
    elif abs(cvr) > abs(aov) * 1.6:
        parts.append("driven mainly by conversion rather than basket size - more "
                     "shoppers are buying, at broadly the same order value")
    else:
        parts.append("with conversion and basket size moving together")

    if len(personas) > 1:
        ranked = sorted(personas, key=lambda p: -(p.get("rpu_lift_pct") or 0))
        top, bottom = ranked[0], ranked[-1]
        spread = (top.get("rpu_lift_pct") or 0) - (bottom.get("rpu_lift_pct") or 0)
        if spread > 8:
            parts.append(f"The response is uneven: {top['persona']} moves "
                         f"{top['rpu_lift_pct']:+.1f}% against {bottom['persona']} at "
                         f"{bottom['rpu_lift_pct']:+.1f}%, so a targeted rollout is "
                         f"likely to beat a blanket one")
        else:
            parts.append("All segments respond similarly, so there is little to gain "
                         "from targeting this change")

    # first two clauses join with a comma, later sentences with a full stop
...
def aggregate(df: pd.DataFrame, persona_sizes: Dict[str, int],
              population: int, allocation: Dict[str, int],
    ...
    ------------------------
    Sampling is proportional, so the raw sample already mirrors the population.
    Applying a segment-size weight on top of that would count the large
    segments twice - once through their larger row count, once through their
    larger weight.

    The correct factor is inverse-probability: population share divided by
    sample share. Under proportional sampling that lands near 1.0 and the
    weighting becomes a no-op that only corrects rounding drift. Under an even
    split it restores population proportions properly. One formula, both modes
    correct.
    """
    ...
    sampled_counts = ok["persona"].value_counts().to_dict()
    correction = {}
    for p in sampled_counts:
        pop_share = persona_sizes.get(p, 0) / total_customers
        samp_share = sampled_counts[p] / n_sampled
        correction[p] = (pop_share / samp_share) if samp_share else 0.0
    ok["w"] = ok["persona"].map(correction).fillna(0)
    ...
    # per-shopper absolutes and deltas, for the user-level table and export
    ok["rpu_a"] = ok["cvr_a"] * ok["aov_a"]
    ok["rpu_b"] = ok["cvr_b"] * ok["aov_b"]
    ok["cvr_delta_pts"] = ((ok["cvr_b"] - ok["cvr_a"]) * 100).round(2)
    ok["cvr_lift_pct"] = np.where(ok["cvr_a"] > 0,
        (ok["cvr_b"] - ok["cvr_a"]) / ok["cvr_a"] * 100, 0.0).round(2)
    ok["aov_delta"] = (ok["aov_b"] - ok["aov_a"]).round(2)
    ok["aov_lift_pct"] = np.where(ok["aov_a"] > 0,
        (ok["aov_b"] - ok["aov_a"]) / ok["aov_a"] * 100, 0.0).round(2)
    ok["rpu_delta"] = (ok["rpu_b"] - ok["rpu_a"]).round(3)
    ok["user_lift"] = np.where(ok["rpu_a"] > 0,
        (ok["rpu_b"] - ok["rpu_a"]) / ok["rpu_a"] * 100, 0.0)
    ok["rpu_lift_pct"] = ok["user_lift"].round(2)

    spread = float(ok["user_lift"].std(ddof=1)) if n_sampled > 1 else 0.0
    stderr = spread / math.sqrt(n_sampled) if n_sampled > 1 else 0.0
    ci95 = 1.96 * stderr

    # ---- per persona, absolutes first then changes ----
    g = ok.groupby("persona")
    persona_df = g.agg(
        n=("user_id", "count"),
        avg_cvr_a=("cvr_a", "mean"), avg_cvr_b=("cvr_b", "mean"),
        avg_aov_a=("aov_a", "mean"), avg_aov_b=("aov_b", "mean"),
        avg_rpu_a=("rpu_a", "mean"), avg_rpu_b=("rpu_b", "mean"),
    ).reset_index()

    persona_df["cvr_delta_pts"] = ((persona_df["avg_cvr_b"] - persona_df["avg_cvr_a"]) * 100).round(2)
    persona_df["cvr_lift_pct"] = ((persona_df["avg_cvr_b"] - persona_df["avg_cvr_a"])
                                  / persona_df["avg_cvr_a"].replace(0, np.nan) * 100).round(2)
    persona_df["aov_delta"] = (persona_df["avg_aov_b"] - persona_df["avg_aov_a"]).round(2)
    persona_df["aov_lift_pct"] = ((persona_df["avg_aov_b"] - persona_df["avg_aov_a"])
                                  / persona_df["avg_aov_a"].replace(0, np.nan) * 100).round(2)
    persona_df["rpu_delta"] = (persona_df["avg_rpu_b"] - persona_df["avg_rpu_a"]).round(3)
    persona_df["rpu_lift_pct"] = ((persona_df["avg_rpu_b"] - persona_df["avg_rpu_a"])
                                  / persona_df["avg_rpu_a"].replace(0, np.nan) * 100).round(2)

    for c in ["avg_cvr_a", "avg_cvr_b"]:
        persona_df[c] = (persona_df[c] * 100).round(2)          # shown as percentages
    for c in ["avg_aov_a", "avg_aov_b", "avg_rpu_a", "avg_rpu_b"]:
        persona_df[c] = persona_df[c].round(2)

    persona_df["segment_size"] = persona_df["persona"].map(persona_sizes)
    persona_df["segment_share_pct"] = (
        persona_df["segment_size"] / total_customers * 100).round(1)
    persona_df["sample_share_pct"] = (persona_df["n"] / n_sampled * 100).round(1)
    persona_df["projected_customers"] = (
        persona_df["segment_size"] / total_customers * population).round(0).astype(int)
    persona_df["projected_revenue_delta"] = (
        (persona_df["avg_rpu_b"] - persona_df["avg_rpu_a"])
        * persona_df["projected_customers"]).round(2)
    # a segment this thinly sampled cannot carry a reliable per-segment figure
    persona_df["thin_sample"] = persona_df["n"] < 5

    persona_df = persona_df.sort_values("segment_size", ascending=False)
    persona_records = persona_df.to_dict(orient="records")

    revenue_a, revenue_b = rpu_a * population, rpu_b * population

    summary = {
        "simulated_users": n_sampled,
    ...
    summary["insight"] = _insight(summary, persona_records, noise_floor)

    return {
        "overall_summary": summary,
        "persona_breakdown": persona_records,
...
def run_ab_test(users_csv: str, variant_a: str, variant_b: str,
    ...
    the cost. It is allocated across personas in proportion to their share of
    the customer base. Results are always projected across the whole dataset.
    """
    ...
    available = df["persona_name"].value_counts().to_dict()
    allocation = allocate_sample(sample_size, persona_sizes, available)
    ...
    sampled = sample_users(df, allocation, seed)
    samples = []
    ...
    out = aggregate(pd.DataFrame(results), persona_sizes, pop, allocation,
...
def main():
    ...
    ap.add_argument("--sample-size", type=int, default=None,
                    help=f"total shoppers to simulate, allocated across personas "
                         f"in proportion to segment size (default {DEFAULT_SAMPLE_SIZE})")
    ...
    print(f"\n  {s['simulated_users']} shoppers simulated "
          f"({s['failed_users']} failed), allocated proportionally:")
    for name, n in sorted(s["allocation"].items(), key=lambda x: -x[1]):
        print(f"      {name[:34]:<36}{n:>5}")
