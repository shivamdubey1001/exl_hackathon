...
Takes users_with_personas.csv, samples users per persona, and asks each one -
in character - how they would respond to two variants. Aggregates into the
CVR / AOV / RPU lift table a merchant expects from a real A/B test.

  python synthetic_ab_test.py \
      --users out/users_with_personas.csv \
      --lookup out/persona_lookup.json \
      --variant-a "Standard product page" \
      --variant-b "10% off banner for orders over $100" \
      --out out/ab_result.json

THE A/A TEST - why this matters more than the A/B result
---------------------------------------------------------
An LLM asked the same question twice gives different answers. So if a synthetic
A/B test reports "+4% lift", the honest question is: how much lift does it
report when BOTH variants are identical?

Run with --aa-test and the engine runs variant A against itself. Whatever lift
comes back is pure model noise. Any real result smaller than that number is
meaningless.

No other team will have measured this, and it is the single strongest answer to
"how do we know your simulation is real". Run it once, put the number on a
slide, and report every A/B result against it.

GROUNDING
---------
Each simulated user carries their REAL measured behaviour - conversion rate,
order value, session depth, discount reliance - not just a persona label. The
model is told to predict a CHANGE from that measured baseline rather than
inventing a conversion rate from nothing.
=============================================================================
"""
...
MAX_WORKERS = 4
REQUESTS_PER_SECOND = 3.0     # polite rate limit, tune to your tier
...
# Real behavioural columns -> the metadata block handed to the model.
# Everything here is MEASURED, which is what makes the baseline grounded.
BASELINE_MAP = {
...
def build_user_metadata(row: pd.Series) -> dict:
    ...
    # baseline conversion rate: prefer the measured session conversion rate
...
def sample_users(df: pd.DataFrame, per_persona: int, seed: int = 42) -> pd.DataFrame:
    ...
    Aggregate figures are re-weighted by true segment size afterwards.
    """
    if "persona_name" not in df.columns:
        raise SystemExit("users file has no persona_name column - "
                         "run assign_personas.py first")
...
def evaluate_user(sample: dict, variant_a: str, variant_b: str,
    ...
    On failure this returns failed=True and NO predictions. It deliberately
    does not fall back to baseline-for-both-variants: that produces exactly
    zero lift, which is indistinguishable from a real null result and would
    quietly poison the aggregate.
    """
    import anthropic
...
def aggregate(df: pd.DataFrame, persona_sizes: Dict[str, int]) -> dict:
    """
    Aggregate to headline lift figures.

    Sampling is balanced across personas, so the raw mean would misrepresent a
    base where one persona holds 58% of customers. Everything is re-weighted by
    true segment size.
    """
    ok = df[~df["failed"]].copy()
    if ok.empty:
        return {"error": "every simulated user failed", "failures": len(df)}

    total = sum(persona_sizes.values())
    weights = {p: persona_sizes.get(p, 0) / total for p in ok["persona"].unique()}
    ok["w"] = ok["persona"].map(weights).fillna(0)

    def wmean(col):
        return float(np.average(ok[col], weights=ok["w"])) if ok["w"].sum() else float(ok[col].mean())
    ...
    # per-user relative lift, for a spread estimate
    ok["user_rpu_a"] = ok["cvr_a"] * ok["aov_a"]
    ok["user_rpu_b"] = ok["cvr_b"] * ok["aov_b"]
    ok["user_lift"] = np.where(ok["user_rpu_a"] > 0,
                               (ok["user_rpu_b"] - ok["user_rpu_a"]) / ok["user_rpu_a"] * 100,
                               0.0)
    n = len(ok)
    spread = float(ok["user_lift"].std(ddof=1)) if n > 1 else 0.0
    stderr = spread / np.sqrt(n) if n > 1 else 0.0
...
# =============================================================================
# ORCHESTRATION - importable by the frontend
# =============================================================================
...
def run_ab_test(users_csv: str, variant_a: str, variant_b: str,
                ...
                progress=None) -> dict:
    """
    Single entry point. The Streamlit frontend should call this directly rather
    than shelling out, so variants come from a text box, not the source file.
    """
    df = pd.read_csv(users_csv)
    persona_sizes = df["persona_name"].value_counts().to_dict()
    n_personas = df["persona_name"].nunique()

    # default: n per persona, where n = number of personas
    if per_persona is None:
        per_persona = n_personas
    ...
    out = aggregate(pd.DataFrame(results), persona_sizes)
    out["config"] = {
        "variant_a": variant_a,
        "variant_b": b_desc,
        "aa_test": aa_test,
...
def main():
    ...
    ap.add_argument("--per-persona", type=int, default=None,
                    help="users sampled per persona (default: number of personas)")
    ap.add_argument("--model", default=DEFAULT_MODEL)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--aa-test", action="store_true",
                    help="run variant A against itself to measure model noise")
    ap.add_argument("--no-character", action="store_true",
                    help="skip persona system prompts, use metadata only")
    ...
    res = run_ab_test(
        ...
        per_persona=args.per_persona, lookup_path=args.lookup,
    ...
    print(f"\n  simulated {s['simulated_users']} shoppers "
          f"({s['failed_users']} failed)\n")
    ...
    print(f"\n  spread across shoppers: +/- {s['lift_std_dev_pct']:.2f}pp "
          f"(std error {s['lift_std_error_pct']:.2f}pp)")
    ...
    quotes = [r for r in res["raw_results"] if r.get("quote")]
    if quotes:
        print("\n--- VOICES ---")
        seen = set()
        for r in quotes:
            if r["persona"] in seen:
                continue
            seen.add(r["persona"])
            print(f"  [{r['persona']}] \"{r['quote']}\"")
