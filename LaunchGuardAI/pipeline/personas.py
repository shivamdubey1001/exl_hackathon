...
def call_llm(system: str, user: str, model: str) -> str:
    ...
    for attempt in range(3):
        try:
            ...
            # stop_reason tells us whether the model finished or was cut off.
            # "max_tokens" means raise the limit; "end_turn" on broken JSON
            # means the model genuinely stopped mid-object and needs a retry.
            if r.stop_reason != "end_turn":
                print(f"[personas] stop_reason={r.stop_reason} len={len(text)}")
            return text
...
def generate_personas(meta: dict, model: str = DEFAULT_MODEL,
    ...
    for i, cluster in enumerate(clusters):
        ...
        for attempt in range(3):
            raw = ""
            try:
                raw = call_llm(SYSTEM_INSTRUCTION, prompt, model)
