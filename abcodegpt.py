...
# ============================================
# Paste your OpenAI API key below, OR set it as an environment variable
# named OPENAI_API_KEY before running this script (recommended).
os.environ["OPENAI_API_KEY"] = ""  # <-- INSERT YOUR OPENAI API KEY HERE

OPENAI_MODEL = "gpt-4o"

# Initialize OpenAI client (reads OPENAI_API_KEY from the environment automatically)
...
def evaluate_user(sample, variant_a_desc, variant_b_desc):
    """Evaluates micro-conversions for a single user using ChatGPT (OpenAI)."""
    payload = {
    ...
    try:
        ...
        print(f"⚠️ OpenAI API Error for user {sample['user_id']}: {e}")  # Print the real error!
...
def run_synthetic_ab_test(samples, variant_a_desc, variant_b_desc, max_workers=1):
    ...
    print(f"Running ChatGPT Monte Carlo simulation across N={len(samples)} profiles...")
