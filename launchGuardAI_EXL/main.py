...
def run_pipeline(did: str, body: ClusterBody, background: BackgroundTasks):
    ...
    if not m.get("mapping"):
        raise HTTPException(400, "confirm the column mapping first")
    if body.generate_personas and not os.getenv("ANTHROPIC_API_KEY"):
        raise HTTPException(400, "No API key found. Add ANTHROPIC_API_KEY to .env, "
                                 "or run with generate_personas = false.")
...
def simulate_abtest(did: str, body: ABBody):
    ...
    if body.aa_test:
        ...
        result["above_noise_floor"] = (
            ...
            > abs(nf.get("rpu_relative_lift_pct", 0)))
...
def ab_estimate(did: str, sample_size: int = 120):
    """
    What an A/B run would cost. Pure arithmetic — the UI calls this on every
    keystroke, so it must not touch the API or do any real work.
    """
    _require_ready(did)
    from synthetic_ab_test import estimate_run
    ...
    est = estimate_run(n_personas, sample_size, total,
                       os.getenv("LAUNCHGUARD_MODEL", "claude-sonnet-5"))
    return {**est, "personas": n_personas, "total_customers": total}
