...
def _call_model(system: str, user: str, model: str) -> tuple:
    """Returns (text, raw_response). Raw response is passed to usage tracking."""
    from usage import make_client
    client = make_client()
    r = client.messages.create(model=model, max_tokens=2000, system=system,
                               messages=[{"role": "user", "content": user}])
    return "".join(b.text for b in r.content if b.type == "text"), r
...
def ai_fill_gaps(df: pd.DataFrame, mapping: Dict[str, Optional[str]],
    ...
    try:
        text, raw = _call_model(AI_SYSTEM, prompt, model)
        try:
            import usage
            usage.record("mapping", model, raw,
                         label=f"{len(missing)} unmatched columns")
        except Exception:
            pass                        # cost tracking must not break mapping
