...
def make_client():
    """
    Anthropic direct, or OpenRouter when an OpenRouter key is present.

    OpenRouter exposes an Anthropic-compatible endpoint, so the same SDK works
    with only a base_url and key swap.
    """
    from anthropic import Anthropic
    or_key = os.getenv("OPENROUTER_API_KEY")
    if or_key:
        return Anthropic(api_key=or_key,
...
def summary() -> dict:
    ...
    return {
        ...
        "window": MAX_LOG,
        "free_steps": [
            {"name": "Fuzzy column matching",
             "why": "String matching runs locally — the model is only asked "
                    "about columns it could not resolve"},
            {"name": "Segmentation",
        ...
        "pricing_note": "Rates checked August 2026. Sonnet 5 is on "
                        "introductory pricing until 31 August 2026.",
    }
