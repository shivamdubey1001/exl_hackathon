...
def is_usable(df: pd.DataFrame, col: str) -> bool:
    """Numeric, present, and actually varies. A constant column adds a
    dimension without adding information."""
    return (col in df.columns
            and pd.api.types.is_numeric_dtype(df[col])
            and df[col].nunique(dropna=True) > 1)


def available_features(df: pd.DataFrame) -> List[dict]:
    """
    Every column the user could cluster on, with a recommendation.

    Drives the feature picker. `locked` marks the outcome columns personas are
    meant to predict - clustering on those leaks the answer into the question,
    so they are shown greyed rather than hidden, which is more honest than
    ...
    for col in df.columns:
        ...
        constant = df[col].nunique(dropna=True) <= 1
        if col in NEVER_CLUSTER:
            group, default, locked = "outcome", False, True
            note = "Held out - this is what the personas are asked to predict"
        elif col in CORE_FEATURES:
            group, default, locked = "core", True, False
            note = "Core behavioural signal"
        elif col in SYNTHETIC:
            group, default, locked = "synthetic", False, False
            note = "Generated overlay - leave off to keep segments emergent"
        else:
            group, default, locked = "extra", False, False
            note = "Describes a segment well, but rarely defines one"

        if constant:
            default, locked = False, True
            note = "Same value for every customer - cannot separate anyone"

        out.append({"column": col, "group": group,
                    "default": default and not constant,
                    "locked": locked, "constant": constant, "note": note})

    order = {"core": 0, "extra": 1, "synthetic": 2, "outcome": 3}
    return sorted(out, key=lambda f: (order[f["group"]], f["column"]))


def default_feature_set(df: pd.DataFrame, include_synthetic: bool = False) -> List[str]:
    """What we would pick if the user expresses no preference."""
    picked = [c for c in CORE_FEATURES if is_usable(df, c)]
    if len(picked) < MIN_FEATURES:
        picked += [c for c in FALLBACK_FEATURES
                   if is_usable(df, c) and c not in picked]
    if include_synthetic:
        picked += [c for c in SYNTHETIC if is_usable(df, c) and c not in picked]
    if len(picked) < MIN_FEATURES:
        for col in df.columns:
            if col in picked or col == "user_id" or col in NEVER_CLUSTER:
                continue
            if is_usable(df, col):
                picked.append(col)
    return picked
...
def prepare_features(df: pd.DataFrame,
                     ...
                     features: Optional[List[str]] = None) -> (pd.DataFrame, List[str]):
    """
    Build the numeric matrix to cluster on, and say which columns it used.

    `features` overrides the automatic choice. Anything unusable or held out is
    dropped from the override rather than honoured, because a user ticking a
    constant column should not silently degrade the model.
    """
    work = df.copy()

    if features:
        candidates = [c for c in features
                      if c not in NEVER_CLUSTER and is_usable(work, c)]
    else:
        candidates = default_feature_set(work, include_synthetic)

    if len(candidates) < 2:
        raise ValueError(
            "Need at least 2 numeric columns that vary across customers. "
            "Select more columns, or check the ones chosen are not constant.")
...
def run_clustering(df: pd.DataFrame, k: int,
    ...
    feats, used = prepare_features(df, include_synthetic, features)
    if len(used) < 2:
    ...
    return {
        ...
        "meta": {
            ...
            "clustered_on": used,
            "feature_choice": "user" if features else "automatic",
